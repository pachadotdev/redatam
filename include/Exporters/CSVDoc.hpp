...
private:
  void WriteVariableLabels() {
    ...
    std::filesystem::path folder =
        std::filesystem::path(Filename).parent_path() / "Labels";
    std::filesystem::create_directories(folder);
    std::string path =
        (folder / std::filesystem::path(Filename).stem()).string() +
        "-VARIABLES.csv";

    CSVDoc labels(path);
    labels.Columns.push_back("variable");
    labels.Columns.push_back("label");
