...
uint32_t fread_PCK(std::istream &stream) {
  uint32_t x = stream.get() + (stream.get() << 8) + (stream.get() << 16) +
               (stream.get() << 24);
  return x;
}

uint32_t fread_BIN(std::istream &stream) {
  uint32_t x = (stream.get() << 24) + (stream.get() << 16) +
               (stream.get() << 8) + stream.get();
  return x;
}
