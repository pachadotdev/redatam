...
inline uint8_t fread_uint8_t(std::istream &stream) { return stream.get(); }

inline uint16_t fread_uint16_t(std::istream &stream) {
  return stream.get() + (stream.get() << 8);
}

inline uint32_t fread_uint32_t(std::istream &stream) {
  return fread_uint16_t(stream) + (fread_uint16_t(stream) << 16);
}

inline void utf8_from_windows1252(std::string &dst, unsigned char c) {
  if (c > 0x7f) {
...
inline size_t fread_decimal(std::istream &stream, size_t digits) {
  size_t result = 0;
...
inline double fread_double(std::istream &stream) {
  double d;
  stream.read((char *)&d, sizeof(double));
  return d;
}

inline uint32_t fread_PCK(std::istream &stream) {
  return fread_uint32_t(stream);
}

inline uint32_t fread_BIN(std::istream &stream) {
