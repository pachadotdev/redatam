...
sexp read_redatam_(strings dic_path_in) {
  ...
  sexp variable_classes[6];
  variable_classes[0] = Rf_mkString("redatam.variable.int");
  variable_classes[1] = Rf_mkString("redatam.variable.lng");
  variable_classes[2] = Rf_mkString("redatam.variable.chr");
  variable_classes[3] = Rf_mkString("redatam.variable.bin");
  variable_classes[4] = Rf_mkString("redatam.variable.dbl");
  variable_classes[5] = Rf_mkString("redatam.variable.pck");
  ...
  for (int entity_count = 1; entity_count < root_entity.num_entities; ++entity_count) {
    ...
    if (!entity.ptr_path.empty()) {
      ...
      if (!ptr_real_basename.empty()) {
        ...
        Rf_setAttrib(column0, description_rsymbol, Rf_mkString(("Parent instance row in " + entity.parent + "(surrogate attribute)\n").c_str()));
        SET_VECTOR_ELT(current_entity, 0, column0);
        SET_STRING_ELT(current_entity_names, 0, Rf_mkChar((entity.parent + "_id").c_str()));
        ...
        Rf_setAttrib(current_entity, description_rsymbol, serialize_to_R(entity));
        ...
        for (size_t i = 0; i < entity.num_vars; ++i) {
          ...
          if (var.declaration) {
            ...
            if (!rbf_real_basename.empty()) {
              ...
              Rf_setAttrib(column, instance_len_rsymbol, Rf_ScalarInteger(var.declaration->size));
              Rf_setAttrib(column, instance_num_rsymbol, Rf_ScalarInteger(num_instances));
              Rf_setAttrib(column, data_path_rsymbol, Rf_mkString(rbf_real_path.as_string().c_str()));

              switch (var.declaration->type) {
                case VariableDescriptor::Declaration::Type::DBL:
                  Rf_classgets(column, variable_classes[4]);
                  break;
                case VariableDescriptor::Declaration::Type::CHR:
                  Rf_classgets(column, variable_classes[2]);
                  break;
                case VariableDescriptor::Declaration::Type::INT:
                  Rf_classgets(column, variable_classes[0]);
                  break;
                case VariableDescriptor::Declaration::Type::LNG:
                  Rf_classgets(column, variable_classes[1]);
                  break;
                case VariableDescriptor::Declaration::Type::BIN:
                  Rf_classgets(column, variable_classes[3]);
                  break;
                case VariableDescriptor::Declaration::Type::PCK:
                  Rf_classgets(column, variable_classes[5]);
                  break;
                default:
                  break;
              }
          ...
          SET_STRING_ELT(current_entity_names, i + 1, Rf_mkChar(var.name.c_str()));
          SET_VECTOR_ELT(current_entity, i + 1, column);

          if (var.labels.size() > 0 && (var.declaration->type == VariableDescriptor::Declaration::Type::LNG ||
                                        var.declaration->type == VariableDescriptor::Declaration::Type::INT ||
                                        var.declaration->type == VariableDescriptor::Declaration::Type::PCK ||
            ...
            for (int i = 0; i < static_cast<int>(levels.size()); ++i) {
              SET_STRING_ELT(levels_rvector, i, Rf_mkChar(levels[i].second.c_str()));
