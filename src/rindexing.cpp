...
void translate_factor(sexp dst, sexp levelkeys) {
  ...
  const int *const last = first + Rf_length(levelkeys);
...
void configure_factor(sexp dst, sexp src) {
  ...
  if (!Rf_isNull(levels) and !Rf_isNull(levelkeys)) {
...
sexp variable_bit_get_rvector(sexp robj, sexp indices, F &&fread_elt) {
  std::string path = CHAR(
      Rf_asChar(STRING_ELT(Rf_getAttrib(robj, Rf_mkString("data.path")), 0)));
  const unsigned int instance_len =
      INTEGER_ELT(Rf_getAttrib(robj, Rf_mkString("instance.len")), 0);
  const int instance_num =
      INTEGER_ELT(Rf_getAttrib(robj, Rf_mkString("instance.num")), 0);
...
[[cpp11::register]] sexp bin_get_rvector(sexp bin, sexp indices) {
  return variable_bit_get_rvector(bin, indices, fread_BIN);
}

[[cpp11::register]] sexp pck_get_rvector(sexp pck, sexp indices) {
  return variable_bit_get_rvector(pck, indices, fread_PCK);
}

[[cpp11::register]] sexp chr_get_rvector(sexp chr, sexp indices) {
...
[[cpp11::register]] sexp int_get_rvector(sexp intg, sexp indices) {
...
[[cpp11::register]] sexp lng_get_rvector(sexp lng, sexp indices) {
  sexp ans = variable_byte_get_rvector(
      lng, indices,
...
[[cpp11::register]] sexp real_get_rvector(sexp real, sexp indices) {
  return variable_byte_get_rvector(
      real, indices,
